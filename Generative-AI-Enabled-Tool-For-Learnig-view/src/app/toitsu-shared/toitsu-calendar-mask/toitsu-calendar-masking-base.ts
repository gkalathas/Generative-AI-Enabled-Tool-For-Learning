export class ToitsuCalendarMaskingBase {
    ...
    public checkValue(onFocus = false) {
        ...
        if (this._overwriteOnInsert && this._input.selectionStart < this._input.value.length && !this.focus) {
            ...
            if (!ToitsuCalendarMaskingBase.isAlpha(this.value.charAt(selectionStart)) && !ToitsuCalendarMaskingBase.isNumeric(this.value.charAt(selectionStart))) {
                selectionStart++;
            }
    ...
    private isInputKeyAcceptable(): boolean {
        if (this._mask.charAt(this._input.selectionStart - 1) === "9"
            && !ToitsuCalendarMaskingBase.isNumeric(this._input.value.charAt(this._input.selectionStart - 1))) {
            return false;
        }
        if (this._mask.charAt(this._input.selectionStart - 1) === "A"
            && !ToitsuCalendarMaskingBase.isAlpha(this._input.value.charAt(this._input.selectionStart - 1))) {
            return false;
        }
    ...
    public maskValue() {
        ...
        for (let i = 0; i < this._mask.length && this.value.length !== i; i++) {
            ...
            if (!ToitsuCalendarMaskingBase.isAlpha(valueChar)
                && !ToitsuCalendarMaskingBase.isNumeric(valueChar)
                && valueChar !== maskChar) {
                this.value = this.value.substring(0, i) + this.value.substring(i + 1);
                i--;
            } else if (maskChar === "9") {
                if (ToitsuCalendarMaskingBase.isNumeric(valueChar)) {
                ...
                if (ToitsuCalendarMaskingBase.isAlpha(this.value.charAt(i))) {
                    maskedValue += valueChar;
                } else {
                    this.value = this.value.substring(0, i) + this.value.substring(i + 1);
                    i--;
